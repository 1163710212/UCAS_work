...
class HParam:
    ...
    :param hparam_dict: key-value pairs of hyperparameters to logs
    :param metric_dict: key-value pairs of metrics to logs
...
class HumanOutputFormat(KVWriter, SeqWriter):
    ...
    :param filename_or_file: the file to write the logs to
    ...
    def write(self, key_values: Dict, key_excluded: Dict, step: int = 0) -> None:
        ...
        for (key, value), (_, excluded) in zip(sorted(key_values.items()), sorted(key_excluded.items())):
            ...
            if excluded is not None and ("stdout" in excluded or "logs" in excluded):
                continue

            elif isinstance(value, Video):
                raise FormatUnsupportedError(["stdout", "logs"], "video")

            elif isinstance(value, Figure):
                raise FormatUnsupportedError(["stdout", "logs"], "figure")

            elif isinstance(value, Image):
                raise FormatUnsupportedError(["stdout", "logs"], "image")

            elif isinstance(value, HParam):
                raise FormatUnsupportedError(["stdout", "logs"], "hparam")
...
def filter_excluded_keys(
    ...
    :param key_values: logs dictionary to be filtered
...
class JSONOutputFormat(KVWriter):
    ...
    :param filename: the file to write the logs to
    """
...
class TensorBoardOutputFormat(KVWriter):
    ...
    :param folder: the folder to write the logs to
    """
...
def make_output_format(_format: str, log_dir: str, log_suffix: str = "") -> KVWriter:
    ...
    :param _format: the requested format to logs to ('stdout', 'logs', 'json' or 'csv' or 'tensorboard')
    :param log_dir: the logging directory
    :param log_suffix: the suffix for the logs file
    :return: the logger
    """
    ...
    if _format == "stdout":
        ...
        return HumanOutputFormat(os.path.join(log_dir, f"logs{log_suffix}.txt"))
    elif _format == "json":
...
class Logger:
    ...
    def record(self, key: str, value: Any, exclude: Optional[Union[str, Tuple[str, ...]]] = None) -> None:
        ...
        :param key: save to logs this key
        :param value: save to logs this value
    ...
    def _do_log(self, args) -> None:
        """
        logs to the requested format outputs

        :param args: the arguments to logs
        """
        for _format in self.output_formats:
...
def configure(folder: Optional[str] = None, format_strings: Optional[List[str]] = None) -> Logger:
    ...
    :param format_strings: the output logging format
        (if None, $SB3_LOG_FORMAT, if still None, ['stdout', 'logs', 'csv'])
    ...
    if format_strings is None:
        format_strings = os.getenv("SB3_LOG_FORMAT", "stdout,logs,csv").split(",")
