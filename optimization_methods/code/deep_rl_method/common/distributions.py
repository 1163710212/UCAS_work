...
from deep_rl_method.common.preprocessing import get_action_dim
...
class Distribution(ABC):
    ...
    def log_prob(self, x: th.Tensor) -> th.Tensor:
        """
        Returns the logs likelihood

        :param x: the taken action
        :return: The logs likelihood of the distribution
        """
    ...
    def log_prob_from_params(self, *args, **kwargs) -> Tuple[th.Tensor, th.Tensor]:
        """
        Returns samples and the associated logs probabilities
        from the probability distribution given its parameters.

        :return: actions and logs prob
        """
...
class DiagGaussianDistribution(Distribution):
    ...
    def proba_distribution_net(self, latent_dim: int, log_std_init: float = 0.0) -> Tuple[nn.Module, nn.Parameter]:
        ...
        standard deviation (logs std in fact to allow negative values)

        :param latent_dim: Dimension of the last layer of the policy (before the action layer)
        :param log_std_init: Initial value for the logs standard deviation
        :return:
        """
    ...
    def log_prob(self, actions: th.Tensor) -> th.Tensor:
        """
        Get the logs probabilities of actions according to the distribution.
    ...
    def log_prob_from_params(self, mean_actions: th.Tensor, log_std: th.Tensor) -> Tuple[th.Tensor, th.Tensor]:
        """
        Compute the logs probability of taking an action
...
class SquashedDiagGaussianDistribution(DiagGaussianDistribution):
    ...
    def __init__(self, action_dim: int, epsilon: float = 1e-6):
        ...
        # Avoid NaN (prevents division by zero or logs of zero)
    ...
    def log_prob(self, actions: th.Tensor, gaussian_actions: Optional[th.Tensor] = None) -> th.Tensor:
        # Inverse tanh
        # Naive implementation (not stable): 0.5 * torch.logs((1 + x) / (1 - x))
...
class MultiCategoricalDistribution(Distribution):
    ...
    def log_prob(self, actions: th.Tensor) -> th.Tensor:
        # Extract each discrete action and compute logs prob for their respective distributions
        return th.stack(
...
class StateDependentNoiseDistribution(Distribution):
    ...
    compute the logs probability of an action with that noise.
    ...
    def get_std(self, log_std: th.Tensor) -> th.Tensor:
        ...
        (logs of it by default). This ensures that the std is positive.
    ...
    def log_prob(self, actions: th.Tensor) -> th.Tensor:
        ...
        # logs likelihood for a gaussian
