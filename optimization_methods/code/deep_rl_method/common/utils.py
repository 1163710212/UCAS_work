...
from deep_rl_method.common.logger import Logger, configure
from deep_rl_method.common.type_aliases import GymEnv, Schedule, TensorDict, TrainFreq, TrainFrequencyUnit
...
def get_latest_run_id(log_path: str = "", log_name: str = "") -> int:
    """
    Returns the latest run number for the given logs name and logs path,
    by finding the greatest number in the directories.

    :param log_path: Path to the logs folder containing several runs.
...
def configure_logger(
    ...
    :param tb_log_name: tensorboard logs
    ...
    if tensorboard_log is not None and SummaryWriter is None:
        raise ImportError("Trying to logs data to tensorboard but tensorboard is not installed.")
