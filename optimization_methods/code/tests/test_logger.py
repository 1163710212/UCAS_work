...
from deep_rl_method import A2C, DQN
from deep_rl_method.common.logger import (
    DEBUG,
    INFO,
    CSVOutputFormat,
    Figure,
...
class LogContent:
    """
    A simple wrapper class to provide a common interface to check content for emptiness and report the logs format
    """
...
def read_log(tmp_path, capsys):
    def read_fn(_format):
        if _format == "csv":
            ...
            return LogContent(_format, (tmp_path / "logs.txt").read_text().splitlines())
...
@pytest.mark.parametrize("_format", ["stdout", "logs", "json", "csv", "tensorboard"])
...
@pytest.mark.parametrize("unsupported_format", ["stdout", "logs", "json", "csv"])
